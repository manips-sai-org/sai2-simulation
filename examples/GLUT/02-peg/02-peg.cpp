...
int main(int argc, char* argv[])
{
    ...
    cDynamicJoint* jointBodyX = linkObject->newJoint(DYN_JOINT_PRISMATIC, DYN_AXIS_X);
    cDynamicJoint* jointBodyY = linkObject->newJoint(DYN_JOINT_PRISMATIC, DYN_AXIS_Y);
    cDynamicJoint* jointBodyZ = linkObject->newJoint(DYN_JOINT_PRISMATIC, DYN_AXIS_Z);

    // create 1 spherical joint
    cDynamicJoint* jointBodyS = linkObject->newJoint(DYN_JOINT_SPHERICAL);

    // set some damping
    jointBodyX->setDamping(5);
    jointBodyY->setDamping(5);
    jointBodyZ->setDamping(5);
    jointBodyS->setDamping(0.1);
